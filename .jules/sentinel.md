...
## 2026-04-04 - Information Exposure in RESTEasy via WebApplicationException
**Vulnerability:** The application leaked internal details by converting standard HTTP errors (like 405 Method Not Allowed or 415 Unsupported Media Type) into 500 Internal Server Errors via `GlobalExceptionMapper`.
**Learning:** In Quarkus RESTEasy, `WebApplicationException` sub-classes like `NotAllowedException` are caught by generic exception mappers if a specific mapper doesn't exist, which can lead to information exposure. Furthermore, when implementing an `ExceptionMapper<WebApplicationException>`, it's critical to use `Response.fromResponse(exception.getResponse())` to retain the original HTTP headers (like the required `Allow` header for 405 responses).
**Prevention:** Always register a custom `ExceptionMapper<WebApplicationException>` that checks if the response already has an entity, and if not, constructs a generic, sanitized JSON response while preserving the original response headers.
