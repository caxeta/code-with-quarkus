...
## 2026-04-20 - Panache Pagination Size Validation Missing
**Vulnerability:** Zero or negative limits can bypass Panache built-in pagination checks causing backend errors or Denial of Service.
**Learning:** `rest-data-panache` has no built in bounds checking for negative numbers for `size` parameter.
**Prevention:** Explicitly enforce both lower bound (e.g. `size <= 0`) and upper limits for endpoints handling lists of data, ideally via a global `ContainerRequestFilter`.
