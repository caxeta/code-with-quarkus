...
## 2026-08-08 - DoS via NullPointerException in RateLimitFilter
**Vulnerability:** The `RateLimitFilter` relied on `request.remoteAddress().host()` to retrieve the client IP and passed it directly to a Caffeine cache (`counts.get(clientIp, ...)`). If the `remoteAddress` or `host` was null (e.g., due to missing or malformed headers behind a proxy like `X-Forwarded-For`), this would throw a `NullPointerException` because Caffeine's `Cache.get` does not accept null keys. This resulted in an unhandled exception (500 error), allowing an attacker to bypass rate limiting or cause a localized Denial of Service (DoS).
**Learning:** External or proxy-provided values like the remote IP address can be null and should never be passed blindly into components like caches that don't support null keys.
**Prevention:** Always check values that derive from HTTP requests (like `remoteAddress`) for null before using them as keys in maps or caches, and provide a safe fallback value (e.g., `"unknown"`) to prevent unhandled exceptions.
## 2026-08-26 - Missing Audit Logging for WebApplicationException
...
## 2026-09-02 - Cross-User Authentication Leak in RevisionListener
**Vulnerability:** The application cached the request-scoped `SecurityIdentity` in an instance variable (`private volatile SecurityIdentity securityIdentity`) inside `CustomRevisionListener`. Since RevisionListeners are singletons or reused across multiple requests, this caused a Cross-User Authentication Leak where a revision triggered by one user could be attributed to a previously cached user.
**Learning:** In Quarkus applications using Hibernate Envers, do not cache request-scoped beans (like `SecurityIdentity`) as instance variables within a `RevisionListener`. Because the listener acts as a singleton, caching request-scoped beans causes a Cross-User Authentication Leak.
**Prevention:** Always retrieve request-scoped dependencies dynamically (e.g., `CDI.current().select(SecurityIdentity.class).get()`) within the method scope and do not store them as instance variables in components that act as singletons or have longer lifecycles.
